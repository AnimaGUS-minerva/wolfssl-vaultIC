...
int WOLFSSL_VAULTIC_LoadCertificates(WOLFSSL_CTX* ctx)
{
    ...
    if ((sizeof_client_cert = vlt_tls_get_cert_size(SSL_VIC_CLIENT_CERT)) == -1) {
        VAULTIC_LOG("ERROR: No Client Certificate found in VaultIC\n");
        return -1;
    }

    client_cert = XMALLOC(sizeof_client_cert, NULL, DYNAMIC_TYPE_ECC_BUFFER);

    if (vlt_tls_read_cert(client_cert, SSL_VIC_CLIENT_CERT) !=0 ) {
        VAULTIC_LOG("ERROR: vlt_tls_read_cert client\n");
        goto free_cert_buffers;
    }

    VAULTIC_LOG("\n[Client certificate]");
    VAULTIC_LOG_BUFFER(client_cert, sizeof_client_cert);

    /* Read CA certificate in VaultIC */
    VAULTIC_LOG("Read CA Certificate in VaultIC\n");
    if ((sizeof_ca_cert = vlt_tls_get_cert_size(SSL_VIC_CA_CERT)) == -1) {
        VAULTIC_LOG("ERROR: No CA Certificate found in VaultIC\n");
        goto free_cert_buffers;
    }
    ...
    if (vlt_tls_read_cert(ca_cert, SSL_VIC_CA_CERT) !=0 ) {
        VAULTIC_LOG("ERROR: vlt_tls_read_cert CA\n");
        goto free_cert_buffers;
    }

    VAULTIC_LOG("\n[CA certificate]");
    VAULTIC_LOG_BUFFER(ca_cert, sizeof_ca_cert);

    /* Load CA certificate into WOLFSSL_CTX */
    if ((ret = wolfSSL_CTX_load_verify_buffer(ctx, ca_cert,
        sizeof_ca_cert, WOLFSSL_FILETYPE_ASN1)) != SSL_SUCCESS) {
        VAULTIC_LOG("ERROR: failed to load CA certificate %s",wc_GetErrorString(ret));
        goto free_cert_buffers;
    }
    ...
    if ((ret = wolfSSL_CTX_use_certificate_buffer(ctx, client_cert,
        sizeof_client_cert, WOLFSSL_FILETYPE_ASN1)) != SSL_SUCCESS) {
